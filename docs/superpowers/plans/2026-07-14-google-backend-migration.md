...
- [ ] **3. Configure the OAuth consent screen** (APIs & Services → OAuth consent screen): User type **External**, publishing status **Testing** is fine for a 2-person family app (no Google review needed as long as it stays in Testing). Add your Google account and your partner's as **Test users**.
- [ ] **4. Create an OAuth 2.0 Client ID** (APIs & Services → Credentials → Create Credentials → OAuth client ID → Web application). Add **two** authorized redirect URIs:
  - `http://localhost:3000/oauth-callback` (used once, by `scripts/get-refresh-token.mjs`)
  - `https://<your-vercel-domain>/api/auth/callback/google` (used by NextAuth in production — also add `http://localhost:3000/api/auth/callback/google` if testing login locally)
...
- NextAuth Google login with allowlist → already fully implemented in `auth.ts`/`middleware.ts`; Task 6 only dedupes the `PREVIEW` flag, no new auth logic needed.
